...
pub fn do_patch_commit(
    ...
    let saved_tree = use_install_tree(manager, &mut lockfile)?;
    ...
    restore_saved_tree(&mut lockfile, saved_tree);
...
pub fn prepare_patch(manager: &mut PackageManager) -> Result<(), crate::Error> {
    ...
    restore_saved_tree(&mut manager.lockfile, saved_tree);
...
fn bundler_on_path(lockfile: &Lockfile, path: &[u8], pkg_id: PackageID) -> Option<PackageID> {
    ...
    match folder_in_install_tree(lockfile, &path, pkg_id) {
...
fn folder_in_install_tree(lockfile: &Lockfile, path: &[u8], pkg_id: PackageID) -> Option<Folder> {
    ...
    while components.peek().is_some() {
        ...
        let placed = placed.filter(|&dep_id| !is_last || resolutions[dep_id as usize] == pkg_id);
...
fn crash_if_bundled_path(lockfile: &Lockfile, path: &[u8], pkg_id: PackageID, name: &[u8]) {
    let bundler = bundler_on_path(lockfile, path, pkg_id)
...
type SavedTree = (tree::List, lockfile::DependencyIDList);

/// Swaps in the tree that the hoisted install builds. The saved tree also places what the tarballs ship.
fn use_install_tree(
    manager: &mut PackageManager,
    lockfile: &mut Lockfile,
) -> Result<SavedTree, crate::Error> {
    let saved_tree = (
        lockfile.buffers.trees.clone(),
        lockfile.buffers.hoisted_dependencies.clone(),
    );
    let log = manager.log_mut();
    lockfile.filter(log, manager, true, &[], None)?;
    Ok(saved_tree)
}

fn restore_saved_tree(lockfile: &mut Lockfile, saved_tree: SavedTree) {
    (
        lockfile.buffers.trees,
        lockfile.buffers.hoisted_dependencies,
    ) = saved_tree;
}
